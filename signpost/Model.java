...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        int last = _width * _height; /** the dimension of the puzzle */
        ...
        for (int i = 0; i < width(); i++) {
            ...
            for (int j = 0; j < _board[i].length; j++) {
                int solution_number = solution[i][j];
                allNums.set(solution_number);

                int sequence_number;
                if (solution_number == 1 || solution_number == width() * height()){
                    sequence_number = solution_number; }
                else{
                    sequence_number = 0; }
                ...
                int dirNum = arrowDirection(i, j);

                boolean isFixed = sequence_number == 1
                        || sequence_number == width() * height();

                column[j] = new Sq(i, j, sequence_number, isFixed, dirNum, group_number);
                column[j]._successors = (solution_number == size())
                        ? null : _allSuccessors[i][j][dirNum];
                column[j]._predecessors = (solution_number == 1)
                        ? null : _allSuccessors[i][j][0];
                column[j]._successor = null;
                column[j]._predecessor = null;
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq: col) {
        ...
        for (int i = 1; i <= size(); i++) {
            if (!allNums.get(i)) {
                throw new IllegalArgumentException("Error: Not All Appeared ");
            }
        } */
